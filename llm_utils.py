...
TASK_MODEL_PREFERENCES = {
    "query_refinement": ["gpt-5.4-mini", "gpt-5-mini", "gpt-5.2", "gpt-5.1"],
    "query_expansion": ["gpt-5.4-mini", "gpt-5-mini", "gpt-5.2", "gpt-5.1"],
    "result_triage": ["gpt-5.4-nano", "gpt-5-nano", "gpt-5.4-mini", "gpt-5-mini"],
    "final_report": ["gpt-5.4", "gpt-5.2", "gpt-5.4-mini", "gpt-5-mini"],
}
...
def choose_model_for_task(task_name: str, fallback_model: str, available_models: Optional[List[str]] = None) -> str:
    """
    Pick a task-specific model when available, otherwise preserve the selected model.
    This keeps routing opt-in and avoids forcing unavailable or unconfigured providers.
    """
    available_models = available_models if available_models is not None else get_model_choices()
    available_by_key = {_normalize_model_name(model): model for model in available_models}
    for preferred in TASK_MODEL_PREFERENCES.get(task_name, []):
        match = available_by_key.get(_normalize_model_name(preferred))
        if match:
            return match
    return fallback_model


def build_model_routing_plan(fallback_model: str, enabled: bool, available_models: Optional[List[str]] = None) -> dict:
    if not enabled:
        return {
            "enabled": False,
            "query_refinement": fallback_model,
            "query_expansion": fallback_model,
            "result_triage": fallback_model,
            "final_report": fallback_model,
        }
    available_models = available_models if available_models is not None else get_model_choices()
    return {
        "enabled": True,
        "query_refinement": choose_model_for_task("query_refinement", fallback_model, available_models),
        "query_expansion": choose_model_for_task("query_expansion", fallback_model, available_models),
        "result_triage": choose_model_for_task("result_triage", fallback_model, available_models),
        "final_report": choose_model_for_task("final_report", fallback_model, available_models),
    }
