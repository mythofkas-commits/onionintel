...
timeout 180 bash -c '
until curl --silent --show-error --socks5-hostname 127.0.0.1:9050 --max-time 15 https://check.torproject.org/api/ip >/dev/null; do
  echo "Waiting for Tor circuit..."
  sleep 5
done
'
...
if [ $? -ne 0 ]; then
  echo "WARNING: Tor SOCKS is listening, but no usable circuit was confirmed yet."
else
